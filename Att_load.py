...
def readATT_Data(*args): 
    ...
    data_dict = {fields["name"][i]: data[:, i] for i in range(len(fields["name"]))}
...
def readATT(*args): 
    ...
    fields, header, sync = readATT_Header(ifile, hfunc, harg) # Read Header information
    ...
    readATT_Data(ifile, hfunc, harg, sync, fields) # Read and save binary data format to pickle file.
